...
// node_modules/vue3-dropzone/dist/index.esm.js
...
var acceptsDefault = import_attr_accept.default.default;
var acceptsFn = acceptsDefault || import_attr_accept.default;
function fileAccepted(file, accept) {
  var isAcceptable = file.type === "application/x-moz-file" || acceptsFn(file, accept);
  return [isAcceptable, isAcceptable ? null : getInvalidTypeRejectionErr(accept)];
}
var getTooLargeRejectionErr = function(maxSize) {
  return {
    code: FILE_TOO_LARGE,
    message: "File is larger than ".concat(maxSize, " bytes")
  };
};
var getTooSmallRejectionErr = function(minSize) {
  return {
    code: FILE_TOO_SMALL,
...
function useDropzone(options) {
    ...
    if (isEvtWithFiles(event)) {
      ...
      Promise.resolve(getFilesFromEvent(event)).then(function(files) {
        ...
        files.forEach(function(file) {
          var _a2 = fileAccepted(file, accept), accepted = _a2[0], acceptError = _a2[1];
  ...
  var composeHandler = function(fn) {
    return optionsRef.value.disabled ? void 0 : fn;
  };
  var composeKeyboardHandler = function(fn) {
    return optionsRef.value.noKeyboard ? void 0 : composeHandler(fn);
  };
  var composeDragHandler = function(fn) {
    return optionsRef.value.noDrag ? void 0 : composeHandler(fn);
  };
  ...
  var fileCount = computed(function() {
    return state.draggedFiles ? state.draggedFiles.length : 0;
  });
  var isDragAccept = computed(function() {
    return fileCount.value > 0 && allFilesAccepted({
  ...
  var isDragReject = computed(function() {
    return fileCount.value > 0 && !isDragAccept.value;
  });
  return __assign(__assign({}, toRefs(state)), { isDragAccept, isDragReject, isFocused: computed(function() {
    return state.isFocused && !optionsRef.value.disabled;
  }), getRootProps, getInputProps, rootRef, inputRef, open: composeHandler(openFileDialog) });
}
export {
  useDropzone
};
